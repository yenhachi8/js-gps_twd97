...
export class MapUtility {
  static canvasToWgs(tapInfo) {
    ...
    const position = Utils.pixelToWgs(
      rotatedResultPixel,
      tapInfo.scale,
      tapInfo.originTwd97Point
    );
    return position;
  }

  //wgs -> twd97
  static projectWgsToCanvas(projectInfo) {
  ...
  static twdToCanvas(projectInfo) {
    const center = projectInfo.center;
    const angle = projectInfo.angle;
  ...
  static calculateDistance(p1, p2, scale) {
    const dx = p2.x - p1.x;
    const dy = p2.y - p1.y;
    return Math.sqrt(dx * dx + dy * dy) * scale;
  }
  static meterToYard(meter) {
